...
7. `get_object` reads The Met's live object endpoint for current display and gallery fields. `find_similar_objects` searches only the published `image` vector and never substitutes text similarity. A narrowly recognized route request can call `get_directions`, which resolves an exact Floor 1 gallery through The Met's live interactive map and uses The Great Hall as the indoor route origin. When the user omits the entrance, the answer discloses that it selected the Fifth Avenue entrance as its starting assumption. The route answer is a fixed server template. Its origin, start, floor, destination, distance, duration, source host, and evidence key are compared deterministically with the typed tool result before release.
...
9. A separate structured grounding call checks atomic claims in model-generated drafts against the same bounded evidence. One rejected draft may be regenerated without new tools. A second failure returns a verification-unavailable answer. Deterministic server templates, including confirmed object 404s and bounded wayfinding, validate every rendered field directly and do not add a probabilistic model check.
