...
Text embeddings default to `intfloat/multilingual-e5-large` through FastEmbed. Gemini remains optional. Provider, model name, and dimensions must match collection metadata before any query embedding is computed. The reranker is the Apache-2.0 `Xenova/ms-marco-MiniLM-L-2-v2`, explicitly registered with FastEmbed, pinned to revision `b84c4fa7efd7b4801931e75773c940f002a494f5`. The first search downloads its weights. Model instances are shared and CPU inference is serialized within each process.
...
Every citation must match this turn's tool evidence, including its quote. For model-generated drafts, the lite grounding check extracts atomic claims and identifies supporting evidence keys. Unsupported claims or invalid citations trigger one regeneration. A second failure returns an honest inability to verify, with no citations and grounding score zero. Grounding uses a model judgment and can be wrong; the deterministic citation gate does not prove semantic correctness. Fixed server templates use typed field-to-evidence validation instead. The confirmed-404 and bounded-wayfinding paths fail closed on any mismatch and avoid a model judgment that cannot add information to the typed result.
