...
The production configuration selects Gemini for the main and lite roles, with role-matched Groq fallbacks enabled only when their credentials are present. If Gemini is used without an associated billing account, Google's current [Gemini API terms](https://ai.google.dev/gemini-api/terms) classify it as an unpaid service: submitted content and generated responses may be used to improve products, and human reviewers may process them. Do not send personal, confidential, or sensitive information through that route. Google's [pricing table](https://ai.google.dev/gemini-api/docs/pricing) marks free-tier content as used for product improvement and paid-tier content as excluded from that use. Recheck the current terms before launch because provider policies can change.
...
The collection index records gallery labels, not corridors or room connections. The captured museum-map HTML is an application shell and does not contain a usable route graph. For a narrowly recognized request to a numbered Floor 1 gallery, `get_directions` queries the backend used by The Met's live interactive map, resolves one exact destination, and uses The Great Hall as the indoor route origin. If the user omits an entrance, the answer discloses that it assumes the Fifth Avenue entrance. It returns only the floor, distance, estimated time, and official live route URL. It does not invent turn-by-turn instructions or claim that the 20,000-object index contains wayfinding data. Other stated origins, ambiguous requests, closed destinations, cross-floor routes, malformed responses, and unavailable results fail closed. Accessibility-specific routing is not offered until the map service exposes a verified contract for it.

## Public demo boundary

Gallery shortcuts use the existing lite model's structured semantic classification and validated numeric entities, rather than whole-question regular expressions. The classifier is probabilistic and can still misclassify a request. A 384-token output budget accommodates the full typed result; a ten-token cap would truncate it. No confidence threshold or latency guarantee is inferred from model output. The bounded response templates currently serve English only; other supported languages use the general verified path. Filtered inventory, combined questions, and unsupported route requirements use that path too, so their latency and coverage differ. Unverified numeric queries generate a `routing_deviation` review event in the audit and warning logs.
...
The September 9 startup change packages retrieval assets in the API image and warms local inference before the server accepts traffic. Readiness now checks model warm-up when enabled. This addresses a confirmed code path that initialized models during a customer's first semantic search. The reported 61.6-second family answer had no per-stage breakdown, so that total cannot be attributed entirely to model loading. New logs distinguish model loading, index checks, queue time, embedding, vector search, and reranking. A new Cloud Run deployment and cold/warm end-to-end measurements are required before claiming the production first-request issue is resolved. One minimum instance reduces idle cold starts but incurs cost and does not eliminate restarts or scale-out delays.

The September 9 local offline-cache check against the configured remote visitor index moved 27.77 seconds of index validation, model loading, and warm-up outside the request. The first and second identical family searches took 6.06 and 3.81 seconds, returning the same eight chunks. Per-request model loading was below 0.03 milliseconds. The first search included 2.60 seconds embedding, 0.20 seconds vector search, and 2.92 seconds reranking; the second included 1.23, 0.17, and 2.35 seconds respectively. These Mac measurements used four model threads, no downloads, and no generation API calls. They establish reuse of prepared models, not Cloud Run answer latency or a p95. Reranking remained above the 1.5-second target on this machine.
...
On September 8, 2026, the exact visitor-policy question about a large suitcase and a bottle of wine required five model calls and 20.25 seconds in a fresh local process after moving simple visitor facts to the lite route. Direct visitor dispatch reduced this to three calls and an estimated $0.000760 while retaining one official citation and a 1.0 grounding score. A prewarmed replay took 5.06 seconds; its provider calls used 2.10 seconds and retrieval used 2.78 seconds, including a 2.15-second rerank outlier. Initializing the local embedding and reranking models took 12.75 seconds outside that measured request. Production therefore needs a warm instance for predictable first-user latency; provider failover cannot remove local model initialization time.

On September 7, 2026, a live Gallery 131 request exposed a single-turn free-tier failure. One lite classification call succeeded, followed by four successful GPT-OSS 120B planning calls totaling 7,463 input tokens plus completion tokens. Three immediate 120B attempts then received HTTP 429. The model was repeatedly gathering live object evidence, and the adapter retried the already exhausted model. The corrective control limits tool selection to two rounds, performs no same-model retry on 429, parses Groq's compact token-reset headers, applies a cooldown, and uses GPT-OSS 20B as a separate capacity reserve for a failed main call. Both models have an 8,000 TPM free limit, so this improves graceful degradation but does not create additional guaranteed capacity. Paid account limits or provisioned throughput remain necessary for sustained public traffic.

The deployed follow-up recognizes the narrow direct-gallery inventory intent after policy classification, runs an exact gallery filter, and validates every candidate against The Met's live object endpoint. It builds the inventory from those validated records and applies the unchanged grounding verifier. A September 8 production replay of “What can I see in Gallery 131?” returned the Sphinx of Hatshepsut and the Temple of Dendur with two object citations, grounding 1.00, and 1.10-second end-to-end latency. Search took 106 ms and the two live object checks took 152 ms and 107 ms. The route made exactly two GPT-OSS 20B calls, both HTTP 200, totaling 1,907 input and 262 output tokens. No retry or fallback occurred. The displayed estimated cost was $0.000222. This measurement validates the direct-gallery path; it does not establish a service-wide latency percentile or remove the need for paid capacity under sustained public traffic.
