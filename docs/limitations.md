...
The collection index records gallery labels, not corridors or room connections. The captured museum-map HTML is an application shell and does not contain a usable route graph. For a narrowly recognized Fifth Avenue entrance request to a numbered Floor 1 gallery, `get_directions` queries the backend used by The Met's live interactive map, resolves one exact destination, and uses The Great Hall as the indoor route origin. It returns only the floor, distance, estimated time, and official live route URL. It does not invent turn-by-turn instructions or claim that the 20,000-object index contains wayfinding data. Ambiguous, closed, cross-floor, malformed, or unavailable results fail closed. Accessibility-specific routing is not offered until the map service exposes a verified contract for it.
