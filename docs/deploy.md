...
Cloud Run injects `PORT`; the image honors it and exposes liveness at `/health` and dependency readiness at `/ready`. The image includes pinned E5 and MiniLM ONNX assets and the locked FastEmbed BM25 assets in `/app/models`, separate from disposable session data. It defaults to `MODEL_CACHE_DIR=/app/models`, `MODELS_OFFLINE=true`, and `STARTUP_WARMUP=true`. Remove any stale service overrides that disable these image defaults. No local `.env` change is needed for the container profile.

FastAPI lifespan validates both index schemas, loads the models, runs local warm-up inference, and prepares provider adapters before accepting requests. Failed preparation stops startup and preserves the prior serving revision when Cloud Run cannot make the new revision ready. `/ready` additionally requires `retrieval_models=true`; `/health` stays a cheap liveness check. Gemini embedding deployments warm only local sparse/reranking models and do not spend embedding quota on a startup probe. Readiness does not establish Gemini or Groq availability.

The Cloud Build deployment enforces four CPUs, 8 GiB memory, one minimum instance, concurrency four, and startup CPU boost. Keeping a minimum instance incurs idle-instance charges; startup boost also has a CPU charge. Extra instances and replacement instances still need warm-up. The default Cloud Run TCP startup probe must allow initialization to finish before its deadline (up to 240 seconds); review startup timing on the deployed revision before shortening it. These controls remove downloads and model initialization from accepted requests but do not guarantee a particular answer latency. PostgreSQL retains audits across revisions and instances. With the default pool maximum of four and three Cloud Run instances, reserve at least twelve application connections plus database administration headroom.

Before the demonstration, inspect `retrieval_warmed` and `startup_warmup_completed` logs, check `/ready`, then submit the family-information question once and again in a new conversation on the same revision. `retrieval_request_timing` separates inference-lock queue time, index validation, model initialization, and search time. `retrieval_timing` further splits embedding, vector search, and reranking. Model-call records separately capture provider, pacing, and retry time. Record client wall time too, since the answer's agent timer excludes platform startup and some request setup.

Run `make retrieval-check ARGS="--cache-dir /path/to/populated/models"` for a repeatable read-only startup and two-search measurement. It uses the existing local configuration, requires local embeddings, disables model downloads, and makes no generation calls. It requires Qdrant connectivity and checks that both searches return the same nonempty set. This local measurement does not replace a deployed end-to-end or concurrency test.
