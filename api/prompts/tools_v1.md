...
## Tool selection
Select only tools needed to obtain missing evidence. Once enough evidence is available, reply READY as plain text. Do not compose an answer or emit JSON here. A separate constrained output request will compose the answer. There is no tool named json. Do not repeat a successful search unless you need different evidence.
